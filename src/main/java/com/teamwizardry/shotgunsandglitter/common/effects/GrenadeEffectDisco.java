...
public class GrenadeEffectDisco implements GrenadeEffect {
	...
	public void renderImpact(@NotNull World world, @NotNull IGrenadeEntity grenade) {
		...
		ParticleSpawner.spawn(glitter, world, new StaticInterp<>(grenade.getPositionAsVector()), 200, 0, (i, build) -> {
			if (hue >= 1) hue = 0;
			else hue += 0.1;
			...
			build.setScaleFunction(new InterpScale(RandUtil.nextFloat(0.1f, 2f), 0));
			build.setDeceleration(new Vec3d(RandUtil.nextDouble(0.5, 1), RandUtil.nextDouble(0.5, 1), RandUtil.nextDouble(0.5, 1)));

			double radius = 5 * RandUtil.nextFloat();
